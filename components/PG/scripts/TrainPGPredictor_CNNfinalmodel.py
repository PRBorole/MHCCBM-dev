...
# Max iteration 65 based on CV results where most iterations are btn 59--65 (let's keep extra)
config['epochs'] = 65
...
train_dataset = ProteinSequenceDataset(X, y)
...
result_df.to_csv(current_dir+'/results/final_model/flank'+str(flank)+'_PGpred_65iter.csv')

if save_model:
    torch.save(model, current_dir+'/results/final_model/flank'+str(flank)+'_PGpred_65iter.pt')
